...
export default function AccountApp(){
 const[state,setState]=useState<State>(()=>isDeviceEntryRoute()&&!initialDeviceCode()?'device-code':'boot');const[mode,setMode]=useState<AuthMode>('login');const[me,setMe]=useState<Me>();const[profiles,setProfiles]=useState<Profile[]>([]);const[profile,setProfile]=useState('');const[page,setPage]=useState<Page>('Account');const[error,setError]=useState('');const[busy,setBusy]=useState(false);const[codes,setCodes]=useState<string[]>([]);const[deviceCode,setDeviceCode]=useState(initialDeviceCode);const generation=useRef(0);
